...
Obsitero is a Zotero plugin for Notero-style syncing into Obsidian.

It creates one markdown file per Zotero parent item, keeps `My Notes` intact, syncs child notes from the Zotero `Notes` pane, and generates a Dataview index page for browsing your library in Obsidian.

## Current Scope

- One Zotero parent item syncs to one Obsidian markdown file
- Child notes under the Zotero item `Notes` pane are appended into the same file
- `My Notes`, `code`, and `page` remain Obsidian-owned
- Dataview `_Index.md` is generated for database-style browsing
- Sync is one-way: Zotero to Obsidian

## Install

Build output is written to [`dist/obsitero.xpi`](./dist/obsitero.xpi). Install it in Zotero via `Tools -> Plugins -> Install Plugin From File...`.
...
> [!tip]
> This repository started from `zotero-plugin-template`, but now contains the Obsitero plugin implementation.
